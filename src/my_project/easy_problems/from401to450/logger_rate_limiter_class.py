...
class Logger:
    ...
    def __init__(self):
        if Logger.__name_of_instance != 'SolutionOne':
            raise Exception('This is a singleton class')
        else: 
            Logger.__name_of_instance = self 

        self.logger_dict = defaultdict(int)

    @abstractmethod
    def get_instance(self) -> None: 
        print(Logger.__name_of_instance)
...
class LoggerTwo:
    ...
    def shouldPrintMessage(self, timestamp, message):
        ...
        if timestamp - self.time_dict[message] >= 10:
            self.time_dict[message] = timestamp
            return True
        else:
            return False
...
def factory(name:str='SolutionOne') -> Union[Logger,LoggerTwo]:

    localizers = {
        'SolutionOne':Logger,
        'SolutionTwo':LoggerTwo
    }

    return localizers.get(name,Logger)()
