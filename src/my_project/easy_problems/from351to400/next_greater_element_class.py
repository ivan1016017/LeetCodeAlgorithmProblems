...
class Solution:
    ...
    def nextGreaterElement(self, nums1: List[int], nums2: List[int]) -> List[int]:
        ...
        len_nums_1 = len(nums1)
        len_nums_2 = len(nums2)
        solution = [-1 for _ in range(len_nums_1)]

        for i in range(len_nums_1):

            index = nums2.index(nums1[i])

            for j in range(index+1, len_nums_2):
...
print(factory('dsfsdf').nextGreaterElement(nums1 = [4,1,2], nums2 = [1,3,4,2]))
