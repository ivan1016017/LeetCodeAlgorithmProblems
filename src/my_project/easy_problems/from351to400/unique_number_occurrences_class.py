...
class Solution:
    ...
    def uniqueOccurrences(self, arr: List[int]) -> bool:
        ...
        for i in range(len_arr):
            solution_dict[arr[i]] += 1

        return len(solution_dict.values()) == len(set(solution_dict.values()))

class SolutionTwo:
    def uniqueOccurrences(self, arr: List[int]) -> bool:

        C = Counter(arr)  # O(n)
        occurrences = C.values()  #O(n)
...
print(factory('sdfsd').uniqueOccurrences(arr = [1,2,2,1,1,3]))
# print(factory('sdfsd').uniqueOccurrences(arr = [1,2,2,1,1,3]))
