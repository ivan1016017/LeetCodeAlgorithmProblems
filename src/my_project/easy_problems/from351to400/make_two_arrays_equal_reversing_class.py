...
class Solution:
    ...
    def canBeEqual(self, target: List[int], arr: List[int]) -> bool:
        ...
        target_list = [0 for _ in range(1001)]
        arr_list = [0 for _ in range(1001)]

        for i in range(len(target)):
            target_list[target[i]] += 1
            arr_list[arr[i]] += 1 

        for i in range(1001):
            if target_list[i] != arr_list[i]:
                return False 
        return True 
...
print(factory('asdfsdf').canBeEqual(target = [1,2,3,4], arr = [2,4,1,3]))
