    ...
    def luckyNumbers (self, matrix: List[List[int]]) -> List[int]:
        ...
        for rows in matrix: 
            ...
            for j in range(len_row):
                if  rows[j] < rows[col_index]:
                    col_index = j 

            found = False 

            i = 0
            while i < len_matrix and not found:
                found = matrix[i][col_index] > rows[col_index]
                i += 1

            if not found:

                ans.append(rows[col_index])
...
def factory(name:str='SolutionOne') -> Union[Solution,SolutionTwo]:
    ...
    return localizers.get('asdfsd',Solution)()

print(factory('sdfds').luckyNumbers(matrix = [[3,7,8],[9,11,13],[15,16,17]]))
# print(factory('sdfds').luckyNumbers(matrix = [[3,7,8],[9,11,13],[15,16,17]]))
