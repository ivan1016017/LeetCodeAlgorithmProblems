def compute_leaf_scores(domain_scores: dict[str, int]) -> dict[str, int]:
    ...
    # ── 2. Collect ancestors present in the input for each leaf ──────────────
    def ancestors_in_input(domain: str) -> list[str]:
        """Return every suffix of `domain` (split on '.') that exists in the input."""
        parts = domain.split(".")
        return [
            ".".join(parts[i:])
            for i in range(1, len(parts))          # skip the domain itself
            if ".".join(parts[i:]) in all_domains
        ]

    # ── 3. Sum scores ─────────────────────────────────────────────────────────
    result = {}
    for leaf in sorted(leaves):                    # sorted for deterministic output
